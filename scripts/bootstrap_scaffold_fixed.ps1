<## 
  bootstrap_scaffold.ps1
  (auto-generated - writes scaffold files into OutDir, optional git actions)
##>
...
Write-Host "Bootstrap RoastArena scaffold - will create files into: $OutDir" -ForegroundColor Cyan
...
if (-not $OutDirFull) {
    ...
    $OutDirFull = Resolve-Path -LiteralPath $OutDir
}
$OutDirFull = $OutDirFull.ProviderPath

if (-not $NoGit) {
    if (-not (Test-Path (Join-Path $OutDirFull '.git'))) {
        Write-Host "No .git found in target folder. The script will initialize a git repo there if git actions are enabled." -ForegroundColor Yellow
    }
}
...
"@
    ...
    'web/components/ui/HaloButton.tsx' = @"
...
"@
    ...
    'web/components/ui/BentoGrid.tsx' = @"
...
"@
    ...
    'web/app/(main)/pizzeria/page.tsx' = @"
import React from 'react';
import HaloButton from '../../components/ui/HaloButton';
...
"@
    ...
    'backend/functions/ai-roast/index.ts' = @"
/**
...
# Write files into OutDirFull (relative keys)
foreach ($path in $files.Keys) {
    $targetPath = Join-Path $OutDirFull $path
    $fullDir = Split-Path $targetPath -Parent
    if ($fullDir -and -not (Test-Path $fullDir)) {
        New-Item -ItemType Directory -Path $fullDir -Force | Out-Null
    }
    if ((Test-Path $targetPath) -and -not $Force) {
        $ans = Read-Host "File '$targetPath' exists — overwrite? (y/N)"
        if ($ans -notin @('y', 'Y')) { Write-Host "Skipping $targetPath"; continue }
    }
    $content = $files[$path]
...
if (-not $NoGit) {
    ...
    try {
        ...
        Write-Host "Pulling remote (if exists) and rebasing..."
        try { git pull --rebase origin main } catch { Write-Host "Pull failed or remote missing - continuing" }

        Write-Host "Pushing to origin/main..."
        try { git push -u origin main } catch { Write-Host "Push failed - check remote and auth" -ForegroundColor Red }
    } finally {
        Pop-Location
    }
} else {
    Write-Host "Git actions skipped (NoGit specified). Files written to $OutDirFull" -ForegroundColor Yellow
}
